...
func (r *Postgres) CreateResponseMessage(ctx context.Context, postID int64, replyToEmail, message, userAgent string) (domain.Message, error) {
	const query = `
...
`
	...
	err := r.db.QueryRowContext(ctx, query, message, postID, replyToEmail, userAgent).Scan(
		&out.ID,
		&out.PostID,
		&out.Message,
		&out.Email,
